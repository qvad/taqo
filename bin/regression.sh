...
python3 src/runner.py collect --model=$model --config=$config --revision=$rev_1 --output=reg_$model$rev_1

echo "Evaluating test against $rev_2"
python3 src/runner.py collect --model=$model --config=$config --revision=$rev_1 --output=reg_$model$rev_2
